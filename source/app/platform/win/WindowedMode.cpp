...
_rwD3D9CameraClear(void *camera, void *color, RwInt32 clearFlags)
{
    ...
    // WindowedMode fix (CDxHandler::MainCameraRebuildRaster) //
    if (auto* const cam = (RwCamera*)(camera); camera == Scene.m_pRwCamera) {
        IDirect3DSurface9* pSurface;
        D3DSURFACE_DESC stSurfaceDesc;

        GetD3DDevice()->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pSurface);
...
void notsa::InjectWindowedModeHooks() {
    ...
    RH_ScopedCategory("RW");

    RH_ScopedGlobalInstall(_rwD3D9CameraClear, 0x7F7730);
    RH_ScopedGlobalInstall(D3D9SetPresentParameters, 0x7F6CB0);
}
