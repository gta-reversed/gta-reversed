...
void CGangs::InjectHooks() {
    ...
    RH_ScopedInstall(Load, 0x5D3A90, {.enabled = true });
    RH_ScopedInstall(Save, 0x5D3A60, {.enabled = true });
...
bool CGangs::Load() {
    for (auto& gang : Gang) {
        CGenericGameStorage::LoadDataFromWorkBuffer_Org(&gang, sizeof(CGangInfo));
    }
    return true;
}
