...
void CControllerConfigManager::ClearPedMappings(eControllerAction action, KeyCode button, eControllerType type) {
    ...
    if (action != eControllerAction::CA_PED_FIRE_WEAPON_ALT
...
void CControllerConfigManager::SetControllerKeyAssociatedWithAction(eControllerAction action, KeyCode button, eControllerType type) {
    ...
    m_Actions[action].Keys[type].m_uiSetOrder = (eContSetOrder)((uint32)numOfSettings + 1);
}
...
void CControllerConfigManager::ClearVehicleMappings(eControllerAction nop, KeyCode button, eControllerType type) {
    ...
    CheckAndClear(eControllerAction::CA_VEHICLE_LOOK_LEFT, type, button);
    CheckAndClear(eControllerAction::CA_VEHICLE_LOOK_RIGHT, type, button);
    CheckAndClear(eControllerAction::CA_VEHICLE_LOOK_BEHIND, type, button);
    CheckAndClear(eControllerAction::CA_VEHICLE_MOUSE_LOOK, type, button);
    CheckAndClear(eControllerAction::CA_VEHICLE_TURRET_LEFT, type, button);
    CheckAndClear(eControllerAction::CA_VEHICLE_TURRET_RIGHT, type, button);
    CheckAndClear(eControllerAction::CA_VEHICLE_TURRET_UP, type, button);
    CheckAndClear(eControllerAction::CA_VEHICLE_TURRET_DOWN, type, button);
}
...
void CControllerConfigManager::HandleButtonRelease(KeyCode button, eControllerType type, CControllerState* state) {
    ...
    CheckAndReset(eControllerAction::CA_VEHICLE_MOUSE_LOOK, type, button, state->m_bVehicleMouseLook);
}
...
void CControllerConfigManager::AffectControllerStateOn_ButtonDown_Driving(KeyCode button, eControllerType type, CControllerState* state) {
    ...
    CheckAndSetButton(eControllerAction::CA_VEHICLE_LOOK_BEHIND, type, button, state->LeftShoulder2);
    CheckAndSetButton(eControllerAction::CA_VEHICLE_LOOK_BEHIND, type, button, state->RightShoulder2);

    CheckAndSetButton(eControllerAction::CA_VEHICLE_LOOK_LEFT, type, button, state->LeftShoulder2);
    CheckAndSetButton(eControllerAction::CA_VEHICLE_LOOK_RIGHT, type, button, state->RightShoulder2);
    CheckAndSetButton(eControllerAction::CA_VEHICLE_MOUSE_LOOK, type, button, state->m_bVehicleMouseLook);
    ...
    CheckAndSetStick(eControllerAction::CA_VEHICLE_TURRET_LEFT, type, button, state->RightStickX, m_bStickR_X_Rgh_Lft_MovementBothDown[type], -128);
    CheckAndSetStick(eControllerAction::CA_VEHICLE_TURRET_RIGHT, type, button, state->RightStickX, m_bStickR_X_Rgh_Lft_MovementBothDown[type], 128);
    CheckAndSetStick(eControllerAction::CA_VEHICLE_TURRET_UP, type, button, state->RightStickY, m_bStickR_Up_Dwn_MovementBothDown[type], 128);
    CheckAndSetStick(eControllerAction::CA_VEHICLE_TURRET_DOWN, type, button, state->RightStickY, m_bStickR_Up_Dwn_MovementBothDown[type], -128);
}
...
bool CControllerConfigManager::LoadSettings(FILESTREAM file) {
    ...
    auto actionId = 0u;
    for (const auto& type : CONTROLLER_TYPES_ALL) {
        for (auto action = 0u; action < CONTROLLER_ACTION_COUNT; action++) {
...
bool CControllerConfigManager::SaveSettings(FILESTREAM file) {
    ...
    for (auto type : CONTROLLER_TYPES_ALL) {
        for (int32 actionId = 0; actionId < CONTROLLER_ACTION_COUNT; actionId++) {
            // Write action ID
...
void CControllerConfigManager::InitDefaultControlConfiguration() {
    ...
    SetControllerKeyAssociatedWithAction(eControllerAction::CA_PED_FIRE_WEAPON_ALT, RsKeyCodes(notsa::IsFixBugs() ? '0' : '\\'), eControllerType::KEYBOARD);
    ...
    // Fixes key mapping issue between old Windows 98 and modern systems
    SetControllerKeyAssociatedWithAction(eControllerAction::CA_PED_CENTER_CAMERA_BEHIND_PLAYER, RsKeyCodes(notsa::IsFixBugs() ? '3' : '#'), eControllerType::KEYBOARD);
    ...
    if (FrontEndMenuManager.m_nTextLanguage == 1) {
        ...
        SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_LOOK_LEFT, (RsKeyCodes)'A', eControllerType::KEYBOARD);
        ...
        SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_LOOK_LEFT, (RsKeyCodes)'Q', eControllerType::KEYBOARD);
    ...
    SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_LOOK_RIGHT, (RsKeyCodes)'E', eControllerType::KEYBOARD);
    SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_TURRET_LEFT, rsPADLEFT, eControllerType::KEYBOARD);
    SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_TURRET_RIGHT, rsPADRIGHT, eControllerType::KEYBOARD);
    SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_TURRET_UP, rsPADDOWN, eControllerType::KEYBOARD);
    SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_TURRET_DOWN, rsPADUP, eControllerType::KEYBOARD);
    SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_TURRET_UP, rsEND, eControllerType::OPTIONAL_EXTRA_KEY);
    SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_TURRET_DOWN, rsDEL, eControllerType::OPTIONAL_EXTRA_KEY);
    ...
    if (notsa::IsFixBugs()) {
        SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_LOOK_BEHIND, rsPAD5, eControllerType::KEYBOARD);
        SetControllerKeyAssociatedWithAction(eControllerAction::CA_VEHICLE_MOUSE_LOOK, '*', eControllerType::KEYBOARD);
    }
...
void CControllerConfigManager::InitDefaultControlConfigJoyPad(uint32 buttonCount) {
    ...
    constexpr ButtonMapping specificMappings[mappingCount] = {
        ...
        { JOYBUTTON_SIX,       eControllerAction::CA_VEHICLE_LOOK_RIGHT                 },
        { JOYBUTTON_FIVE,      eControllerAction::CA_PED_CYCLE_WEAPON_LEFT             },
        { JOYBUTTON_FIVE,      eControllerAction::CA_VEHICLE_LOOK_LEFT                  },
...
void CControllerConfigManager::InitDefaultControlConfigMouse(const CMouseControllerState& MouseSetUp, bool bMouseControls) {
    ...
    if (MouseSetUp.isMouseRightButtonPressed || isForcedMouseBlinding) {
        ...
        SetMouseButtonAssociatedWithAction(eControllerAction::CA_VEHICLE_MOUSE_LOOK,         rsMOUSE_RIGHT_BUTTON);
    }
    if (MouseSetUp.isMouseMiddleButtonPressed || isForcedMouseBlinding) {
        SetMouseButtonAssociatedWithAction(eControllerAction::CA_VEHICLE_LOOK_BEHIND,        rsMOUSE_MIDDLE_BUTTON);
...
void CControllerConfigManager::InitialiseControllerActionNameArray() {
    ...
    GxtCharStrcpy(m_ControllerActionName[eControllerAction::CA_VEHICLE_LOOK_BEHIND], "VEHICLE_LOOKBEHIND"_gxt);
    ...
    GxtCharStrcpy(m_ControllerActionName[eControllerAction::CA_VEHICLE_LOOK_LEFT], "VEHICLE_LOOKLEFT"_gxt);
    GxtCharStrcpy(m_ControllerActionName[eControllerAction::CA_VEHICLE_LOOK_RIGHT], "VEHICLE_LOOKRIGHT"_gxt);
    GxtCharStrcpy(m_ControllerActionName[eControllerAction::CA_VEHICLE_MOUSE_LOOK], "VEHICLE_MOUSELOOK"_gxt);
    ...
    GxtCharStrcpy(m_ControllerActionName[eControllerAction::CA_VEHICLE_TURRET_LEFT], "VEHICLE_TURRETLEFT"_gxt);
    GxtCharStrcpy(m_ControllerActionName[eControllerAction::CA_VEHICLE_TURRET_RIGHT], "VEHICLE_TURRETRIGHT"_gxt);
    GxtCharStrcpy(m_ControllerActionName[eControllerAction::CA_VEHICLE_TURRET_UP], "VEHICLE_TURRETUP"_gxt);
    GxtCharStrcpy(m_ControllerActionName[eControllerAction::CA_VEHICLE_TURRET_DOWN], "VEHICLE_TURRETDOWN"_gxt);
...
void CControllerConfigManager::SetMouseButtonAssociatedWithAction(eControllerAction action, KeyCode button) {
    ...
    m_Actions[action].Keys[eControllerType::MOUSE].m_uiSetOrder = (eContSetOrder)((uint32)numOfSettings + 1);
}
...
const GxtChar* CControllerConfigManager::GetButtonComboText(eControllerAction action) {
    return action == eControllerAction::CA_VEHICLE_LOOK_BEHIND ? TheText.Get("FEC_LBC") : nullptr; // Use Look Left With Look Right.
}
