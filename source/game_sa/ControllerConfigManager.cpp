...
bool CControllerConfigManager::SetKeyForAction(eControllerAction action, CControllerKey::KeyCode button, eControllerType type) {
    ...
    for (auto i = 0u; i < ALL_CONTROLLERS; ++i) {
...
const GxtChar* CControllerConfigManager::GetActionKeyName(eControllerAction action) {
    int32 v3 = 0;
    ResetSettingOrder(action);
    for (auto i = (int32)m_Actions[action].Keys[KEYBOARD].Order; !i; i += 2) {
        if (++v3 >= 2) {
            return 0;
        }
    }
...
const GxtChar* CControllerConfigManager::GetControllerSettingText(eControllerAction action, CControllerKey::KeyOrder priority) {
    for (auto type = 0u; type < ALL_CONTROLLERS; ++type) {
...
void CControllerConfigManager::ClearSettingsAssociatedWithAction(eControllerAction action, eControllerType type) {
    m_Actions[action].Keys[type].Key  = (type == KEYBOARD || type == OPTIONAL_EXTRA_KEY)
         ? rsNULL
         : (RsKeyCodes)(0);
...
int32 CControllerConfigManager::GetMouseButtonAssociatedWithAction(eControllerAction action) {
    return m_Actions[action].Keys[MOUSE].Key;
}
...
void CControllerConfigManager::ResetSettingOrder(eControllerAction ctrlAction) {
    ...
    //for (auto order = 1; order <= ALL_CONTROLLERS; ++order) {
...
bool CControllerConfigManager::LoadSettings(FILESTREAM file) {
    ...
    for ([[maybe_unused]] auto i : std::views::iota(0u, 4u * std::size(m_Actions))) {
        ...
        CFileMgr::Seek(file, ALL_CONTROLLERS * 2, SEEK_CUR);
    }
    ...
    for (auto& action : m_Actions) {
        ...
        CFileMgr::Read(file, &action.Keys, ALL_CONTROLLERS);
    }
    return true;
}
...
void CControllerConfigManager::InitializeDefaultKeyboardAndMouseBindings() {
    SetKeyForAction(CA_GO_FORWARD, rsUP, KEYBOARD);
    SetKeyForAction(CA_GO_BACK, rsDOWN, KEYBOARD);
    SetKeyForAction(CA_GO_LEFT, rsLEFT, KEYBOARD);
    SetKeyForAction(CA_GO_RIGHT, rsRIGHT, KEYBOARD);

    if (FrontEndMenuManager.m_nTitleLanguage == 2) {
        SetKeyForAction(CA_CONVERSATION_YES, (RsKeyCodes)'Z', KEYBOARD);
        SetKeyForAction(CA_PED_SNIPER_ZOOM_IN, (RsKeyCodes)'Y', OPTIONAL_EXTRA_KEY);
    } else {
        SetKeyForAction(CA_CONVERSATION_YES, (RsKeyCodes)'Y', KEYBOARD);
        SetKeyForAction(CA_PED_SNIPER_ZOOM_IN, (RsKeyCodes)'X', OPTIONAL_EXTRA_KEY);
    }

    if (FrontEndMenuManager.m_nTitleLanguage == 1) {
        SetKeyForAction(CA_GO_FORWARD, (RsKeyCodes)'Z', OPTIONAL_EXTRA_KEY);
        SetKeyForAction(CA_GO_LEFT, (RsKeyCodes)'Q', OPTIONAL_EXTRA_KEY);
        SetKeyForAction(CA_PED_CYCLE_WEAPON_LEFT, (RsKeyCodes)'A', KEYBOARD);
        SetKeyForAction(CA_PED_SNIPER_ZOOM_OUT, (RsKeyCodes)'W', OPTIONAL_EXTRA_KEY);
    } else {
        SetKeyForAction(CA_GO_FORWARD, (RsKeyCodes)'W', OPTIONAL_EXTRA_KEY);
        SetKeyForAction(CA_GO_LEFT, (RsKeyCodes)'A', OPTIONAL_EXTRA_KEY);
        SetKeyForAction(CA_PED_CYCLE_WEAPON_LEFT, (RsKeyCodes)'Q', KEYBOARD);
        SetKeyForAction(CA_PED_SNIPER_ZOOM_OUT, (RsKeyCodes)'Z', OPTIONAL_EXTRA_KEY);
    }

    SetKeyForAction(CA_GO_BACK, (RsKeyCodes)'S', OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_GO_RIGHT, (RsKeyCodes)'D', OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_PED_CYCLE_WEAPON_RIGHT, (RsKeyCodes)'E', KEYBOARD);
    SetKeyForAction(CA_PED_CYCLE_WEAPON_RIGHT, rsPADENTER, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_PED_CYCLE_WEAPON_LEFT, rsPADDEL, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_PED_FIRE_WEAPON, rsPADINS, KEYBOARD);
    SetKeyForAction(CA_PED_FIRE_WEAPON, rsLCTRL, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_PED_SNIPER_ZOOM_IN, rsPGUP, KEYBOARD);
    SetKeyForAction(CA_PED_SNIPER_ZOOM_OUT, rsPGDN, KEYBOARD);
    SetKeyForAction(CA_PED_FIRE_WEAPON_ALT, (RsKeyCodes)'\\', KEYBOARD);
    SetKeyForAction(CA_GROUP_CONTROL_FWD, (RsKeyCodes)'G', KEYBOARD);
    SetKeyForAction(CA_GROUP_CONTROL_BWD, (RsKeyCodes)'H', KEYBOARD);
    SetKeyForAction(CA_CONVERSATION_NO, (RsKeyCodes)'N', KEYBOARD);
    SetKeyForAction(CA_VEHICLE_ENTER_EXIT, (RsKeyCodes)'F', OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_ENTER_EXIT, rsENTER, KEYBOARD);
    SetKeyForAction(CA_CAMERA_CHANGE_VIEW_ALL_SITUATIONS, (RsKeyCodes)'V', KEYBOARD);
    SetKeyForAction(CA_CAMERA_CHANGE_VIEW_ALL_SITUATIONS, rsHOME, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_PED_JUMPING, rsLSHIFT, KEYBOARD);
    SetKeyForAction(CA_PED_JUMPING, rsRCTRL, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_PED_SPRINT, (RsKeyCodes)' ', KEYBOARD);
    SetKeyForAction(CA_PED_DUCK, (RsKeyCodes)'C', KEYBOARD);
    SetKeyForAction(CA_PED_ANSWER_PHONE, rsTAB, KEYBOARD);
    SetKeyForAction(CA_PED_WALK, rsLALT, KEYBOARD);
    SetKeyForAction(CA_PED_LOCK_TARGET, rsDEL, KEYBOARD);
    SetKeyForAction(CA_PED_LOCK_TARGET, rsCAPSLK, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_PED_1RST_PERSON_LOOK_LEFT, rsPADLEFT, KEYBOARD);
    SetKeyForAction(CA_PED_1RST_PERSON_LOOK_RIGHT, rsPADRIGHT, KEYBOARD);
    SetKeyForAction(CA_PED_1RST_PERSON_LOOK_UP, rsPADDOWN, KEYBOARD);
    SetKeyForAction(CA_PED_1RST_PERSON_LOOK_DOWN, rsPADUP, KEYBOARD);
    SetKeyForAction(CA_PED_CENTER_CAMERA_BEHIND_PLAYER, (RsKeyCodes)'#', KEYBOARD);
    SetKeyForAction(CA_PED_LOOKBEHIND, rsPADEND, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_STEER_UP, rsUP, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_STEER_DOWN, rsDOWN, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_RADIO_STATION_UP, rsINS, KEYBOARD);

    if (FrontEndMenuManager.m_nTitleLanguage == 1) {
        SetKeyForAction(CA_VEHICLE_STEER_LEFT, (RsKeyCodes)'Q', KEYBOARD);
        SetKeyForAction(CA_VEHICLE_LOOK_LEFT, (RsKeyCodes)'A', KEYBOARD);
        SetKeyForAction(CA_VEHICLE_RADIO_STATION_UP, (RsKeyCodes)'`', OPTIONAL_EXTRA_KEY);
        SetKeyForAction(CA_TOGGLE_SUBMISSIONS, (RsKeyCodes)233, KEYBOARD);
        SetKeyForAction(CA_VEHICLE_ACCELERATE, (RsKeyCodes)'Z', KEYBOARD);
    } else {
        SetKeyForAction(CA_VEHICLE_STEER_LEFT, (RsKeyCodes)'A', KEYBOARD);
        SetKeyForAction(CA_VEHICLE_LOOK_LEFT, (RsKeyCodes)'Q', KEYBOARD);
        SetKeyForAction(CA_VEHICLE_RADIO_STATION_UP, (RsKeyCodes)'4', OPTIONAL_EXTRA_KEY);
        SetKeyForAction(CA_TOGGLE_SUBMISSIONS, (RsKeyCodes)'2', KEYBOARD);
        SetKeyForAction(CA_VEHICLE_ACCELERATE, (RsKeyCodes)'W', KEYBOARD);
    }

    SetKeyForAction(CA_TOGGLE_SUBMISSIONS, rsPLUS, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_FIRE_WEAPON, rsRCTRL, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_FIRE_WEAPON, rsLALT, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_FIRE_WEAPON_ALT, rsLCTRL, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_FIRE_WEAPON_ALT, rsPADINS, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_BRAKE, (RsKeyCodes)'S', KEYBOARD);
    SetKeyForAction(CA_VEHICLE_STEER_RIGHT, (RsKeyCodes)'D', KEYBOARD);
    SetKeyForAction(CA_VEHICLE_STEER_LEFT, rsLEFT, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_STEER_RIGHT, rsRIGHT, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_HORN, rsCAPSLK, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_HORN, (RsKeyCodes)'H', OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_HANDBRAKE, (RsKeyCodes)' ', KEYBOARD);
    SetKeyForAction(CA_VEHICLE_HANDBRAKE, rsRCTRL, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_LOOK_RIGHT, (RsKeyCodes)'E', KEYBOARD);
    SetKeyForAction(CA_VEHICLE_TURRET_LEFT, rsPADLEFT, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_TURRET_RIGHT, rsPADRIGHT, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_TURRET_UP, rsPADDOWN, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_TURRET_DOWN, rsPADUP, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_TURRET_UP, rsEND, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_TURRET_DOWN, rsDEL, OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_VEHICLE_RADIO_TRACK_SKIP, rsF5, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_RADIO_STATION_DOWN, rsDEL, KEYBOARD);
    SetKeyForAction(CA_VEHICLE_RADIO_STATION_DOWN, (RsKeyCodes)'R', OPTIONAL_EXTRA_KEY);
    SetKeyForAction(CA_PED_CYCLE_TARGET_LEFT, (RsKeyCodes)'[', KEYBOARD);
    SetKeyForAction(CA_PED_CYCLE_TARGET_RIGHT, (RsKeyCodes)']', KEYBOARD);
...
void CControllerConfigManager::InitDefaultControlConfiguration(uint32 buttonCount) {
    ...
    auto set_actions = [&](std::initializer_list<std::pair<eControllerAction, RsKeyCodes>> init_list) {
        ...
        for (const auto& action : actions) {
            SetKeyForAction(action.first, action.second, JOY_STICK);
        }
    };
...
int8 CControllerConfigManager::SetMouseButtonAssociatedWithAction(eControllerAction actionID, CControllerKey::KeyCode button) {
    ...
    action.Keys[MOUSE].Key  = button;
    action.Keys[MOUSE].Order = order;
    return order;
}
...
const GxtChar* CControllerConfigManager::GetControllerSettingTextMouse(eControllerAction action) {
    switch (m_Actions[action].Keys[MOUSE].Key) {
...
const GxtChar* CControllerConfigManager::GetControllerSettingTextJoystick(eControllerAction action) {
    ...
    const auto keyCode = (RsKeyCodes)m_Actions[action].Keys[JOY_STICK].Key;
    if (!keyCode) {
        return 0;
    }
...
void CControllerConfigManager::ProcessKeyboardInput() {
    ...
    for (auto i = 0u; i < NUM_OF_CONTROLLER_ACTIONS; ++i) {
        const auto key1 = (RsKeyCodes)(m_Actions[i].Keys[KEYBOARD].Key);
        const auto key2 = (RsKeyCodes)(m_Actions[i].Keys[OPTIONAL_EXTRA_KEY].Key);
        CPad* pad = CPad::GetPad();
        ...
        if (GetIsKeyboardKeyDown(key1)) {
            if (inMenu && key1 != rsNULL && pad) {
                ProcessKeyEvent(key1, KEYBOARD, &pad->PCTempKeyState);
            }
        }

        if (GetIsKeyboardKeyDown(key2)) {
            if (inMenu && key2 != rsNULL && pad) {
                ProcessKeyEvent(key2, OPTIONAL_EXTRA_KEY, &pad->PCTempKeyState);
            }
        }
        ...
        if (!FrontEndMenuManager.m_bMenuActive) {
            if (!GetIsKeyboardKeyDown(key2) && key1 != rsNULL && GetIsKeyboardKeyDown(key1)) {
                if (m_Actions[CA_NETWORK_TALK].Keys[KEYBOARD].Key == key1) {
                    ks->m_bChatIndicated = 0;
                }
                if (m_Actions[CA_VEHICLE_MOUSE_LOOK].Keys[KEYBOARD].Key == key1) {
                    ks->m_bVehicleMouseLook = 0;
                }
            }
            if (!GetIsKeyboardKeyDown(key1) && key2 != rsNULL && GetIsKeyboardKeyDown(key2)) {
                if (m_Actions[CA_NETWORK_TALK].Keys[OPTIONAL_EXTRA_KEY].Key == key2) {
                    ks->m_bChatIndicated = 0;
                }
                if (m_Actions[CA_VEHICLE_MOUSE_LOOK].Keys[OPTIONAL_EXTRA_KEY].Key == key2) {
                    ks->m_bVehicleMouseLook = 0;
                }
            }
        }
...
void CControllerConfigManager::ProcessMouseInput() {
    ...
    for (auto& action : m_Actions) {
        const auto button = action.Keys[MOUSE].Key;
        ...
        if (GetIsMouseButtonUp(button) && button && pad) {
            if (!FrontEndMenuManager.m_bMenuActive) {
                if (m_Actions[CA_NETWORK_TALK].Keys[MOUSE].Key == button) {
                    state->m_bChatIndicated = 0;
                }
                if (m_Actions[CA_VEHICLE_MOUSE_LOOK].Keys[MOUSE].Key == button) {
...
const GxtChar* CControllerConfigManager::GetDefinedKeyByGxtName(eControllerAction action) {
    static thread_local GxtChar s_ButtonName[64]; // 0xB7147C
    if (FrontEndMenuManager.m_bController) {
        if (const auto keyCode = m_Actions[action].Keys[JOY_STICK].Key) {
    ...
    if (m_Actions[action].Keys[JOY_STICK].Key == (RsKeyCodes)(0)) {
...
bool CControllerConfigManager::CheckMouseButtonState(CControllerKey::KeyCode key) {
    const auto* pad = CPad::GetPad();
    switch (key) {
    case rsMOUSE_LEFT_BUTTON: return pad->GetLeftMouse();
    case rsMOUSE_RIGHT_BUTTON: return pad->GetRightMouse();
    case rsMOUSE_MIDDLE_BUTTON: return pad->GetMiddleMouse();
    case rsMOUSE_WHEEL_UP_BUTTON: return pad->GetMouseWheelUp();
    case rsMOUSE_WHEEL_DOWN_BUTTON: return pad->GetMouseWheelDown();
    case rsMOUSE_X1_BUTTON: return pad->GetMouseX1();
    case rsMOUSE_X2_BUTTON: return pad->GetMouseX2();
...
bool CControllerConfigManager::CheckMouseButtonJustUpState(CControllerKey::KeyCode key) {
    const auto* pad = CPad::GetPad();
    switch (key) {
    case rsMOUSE_LEFT_BUTTON: return pad->GetLeftMouseJustDown();
    case rsMOUSE_RIGHT_BUTTON: return pad->GetRightMouseJustDown();
    case rsMOUSE_MIDDLE_BUTTON: return pad->GetMiddleMouseJustDown();
    case rsMOUSE_WHEEL_UP_BUTTON: return pad->GetMouseWheelUpJustDown();
    case rsMOUSE_WHEEL_DOWN_BUTTON: return pad->GetMouseWheelDownJustDown();
    case rsMOUSE_X1_BUTTON: return pad->GetMouseX1JustDown();
    case rsMOUSE_X2_BUTTON: return pad->GetMouseX2JustDown();
    case 0:                         return false; /* unset button */
    default:                        NOTSA_UNREACHABLE("Invalid key ({})", (int32)(key));
    }
}
...
bool CControllerConfigManager::UseDrivingControls() {
    ...
    if (const auto* const plyr = FindPlayerPed()) {
        if (FindPlayerVehicle()) {
            return static_cast<CPed*>(const_cast<CPlayerPed*>(plyr))->GetPedState() == PEDSTATE_DRIVING && !CPad::GetPad(0)->DisablePlayerControls;
        }
    }
    return false;
}
