...
public:
    ...
    static bool LoadDataFromWorkBuffer_Org(void* data, int32 size);
...
static bool LoadDataFromWorkBuffer(T& data) {
    return CGenericGameStorage::LoadDataFromWorkBuffer_Org((void*)&data, sizeof(T));
}
...
static void LoadMultipleDataFromWorkBuffer(Ts*... out) {
    ...
    (CGenericGameStorage::LoadDataFromWorkBuffer_Org((void*)out, sizeof(Ts)), ...); // And now load all data
}
