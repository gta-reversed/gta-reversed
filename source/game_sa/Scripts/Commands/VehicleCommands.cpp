...
void notsa::script::commands::vehicle::RegisterHandlers() {
    ...
    //REGISTER_COMMAND_HANDLER(GET_AVAILABLE_VEHICLE_MOD, GetAvailableVehicleMod);
    //REGISTER_COMMAND_HANDLER(IS_VEHICLE_ATTACHED, IsVehicleAttached);
    //REGISTER_COMMAND_HANDLER(SET_VEHICLE_DIRT_LEVEL, SetVehicleDirtLevel);
    //REGISTER_COMMAND_HANDLER(DOES_VEHICLE_EXIST, DoesVehicleExist);
    //REGISTER_COMMAND_HANDLER(GET_VEHICLE_QUATERNION, GetVehicleQuaternion);
    //REGISTER_COMMAND_HANDLER(GET_CHAR_IN_CAR_PASSENGER_SEAT, GetCharInCarPassengerSeat);
    //REGISTER_COMMAND_HANDLER(IS_VEHICLE_ON_ALL_WHEELS, IsVehicleOnAllWheels);
    //REGISTER_COMMAND_HANDLER(SET_VEHICLE_QUATERNION, SetVehicleQuaternion);
    //REGISTER_COMMAND_HANDLER(SHUFFLE_CARD_DECKS, ShuffleCardDecks);
    //REGISTER_COMMAND_HANDLER(SET_VEHICLE_AREA_VISIBLE, SetVehicleAreaVisible);
    //REGISTER_COMMAND_HANDLER(GET_MAXIMUM_NUMBER_OF_PASSENGERS, GetMaximumNumberOfPassengers);
    //REGISTER_COMMAND_HANDLER(SET_VEHICLE_AIR_RESISTANCE_MULTIPLIER, SetVehicleAirResistanceMultiplier);
    //REGISTER_COMMAND_HANDLER(SET_TAXI_LIGHTS, SetTaxiLights);
    //REGISTER_COMMAND_HANDLER(CHANGE_PLAYBACK_TO_USE_AI, ChangePlaybackToUseAi);
    //REGISTER_COMMAND_HANDLER(SET_FREEBIES_IN_VEHICLE, SetFreebiesInVehicle);
    //REGISTER_COMMAND_HANDLER(ADD_VEHICLE_MOD, AddVehicleMod);
    //REGISTER_COMMAND_HANDLER(REMOVE_VEHICLE_MOD, RemoveVehicleMod);
    //REGISTER_COMMAND_HANDLER(RANDOM_PASSENGER_SAY, RandomPassengerSay);
    //REGISTER_COMMAND_HANDLER(IS_BIG_VEHICLE, IsBigVehicle);
    //REGISTER_COMMAND_HANDLER(VEHICLE_DOES_PROVIDE_COVER, VehicleDoesProvideCover);
    //REGISTER_COMMAND_HANDLER(GET_CURRENT_VEHICLE_PAINTJOB, GetCurrentVehiclePaintjob);
    //REGISTER_COMMAND_HANDLER(SET_PETROL_TANK_WEAKPOINT, SetPetrolTankWeakpoint);
    //REGISTER_COMMAND_HANDLER(GIVE_VEHICLE_PAINTJOB, GiveVehiclePaintjob);
    //REGISTER_COMMAND_HANDLER(GET_NUMBER_OF_PASSENGERS, GetNumberOfPassengers);
    //REGISTER_COMMAND_HANDLER(IS_VEHICLE_TOUCHING_OBJECT, IsVehicleTouchingObject);
    //REGISTER_COMMAND_HANDLER(VEHICLE_CAN_BE_TARGETTED_BY_HS_MISSILE, VehicleCanBeTargettedByHsMissile);
    //REGISTER_COMMAND_HANDLER(CONTROL_MOVABLE_VEHICLE_PART, ControlMovableVehiclePart);
    //REGISTER_COMMAND_HANDLER(SET_VEHICLE_IS_CONSIDERED_BY_PLAYER, SetVehicleIsConsideredByPlayer);
    //REGISTER_COMMAND_HANDLER(SET_VEHICLE_TO_FADE_IN, SetVehicleToFadeIn);
    //REGISTER_COMMAND_HANDLER(SET_PLAYBACK_SPEED, SetPlaybackSpeed);
    //REGISTER_COMMAND_HANDLER(GET_DOOR_ANGLE_RATIO, GetDoorAngleRatio);
    //REGISTER_COMMAND_HANDLER(IS_CAR_IN_AREA_2D, IsCarInArea2d);
    //REGISTER_COMMAND_HANDLER(RESET_VEHICLE_HYDRAULICS, ResetVehicleHydraulics);
    //REGISTER_COMMAND_HANDLER(WINCH_CAN_PICK_VEHICLE_UP, WinchCanPickVehicleUp);
    //REGISTER_COMMAND_HANDLER(IS_EMERGENCY_SERVICES_VEHICLE, IsEmergencyServicesVehicle);
