...
static inline auto& startTimeMs = StaticRef<uint64>(0xB610F8);
static inline auto& m_sfLogLookup = StaticRef<notsa::mdarray<float, 50, 2>>(0xB61100);
...
float CAEAudioUtility::GetPiecewiseLinear(float x, int16 dataCount, const float (*data)[2]) {
    ...
    const float t = (x - data[i - 1][0]) / (data[i][0] - data[i - 1][0]);
    return t * (data[i][1] - data[i - 1][1]) + data[i - 1][1];
}

// 0x4d9e50
float CAEAudioUtility::AudioLog10(float p) {
    return p >= 0.00001f ? std::log10(p) : -5.0f;
}

// NOTSA: std::chrono instead of win32
// 0x4D9E80
...
CVehicle* CAEAudioUtility::FindVehicleOfPlayer() {
    if (auto* vehicle = FindPlayerVehicle(-1, true); vehicle) {
        return vehicle;
    }
    ...
    if (auto* attach = player->m_pAttachedTo; attach && attach->GetIsTypeVehicle()) {
        return attach->AsVehicle();
    }
